 ...
 * 礼品卡订单支付后
 */
class MemberLevelOrderPayWeappDelivery
{
    public function handle($param)
    {
        try{
            $order_model = new MemberLevelOrder();
            $order_condition = array (
                [ 'order_id', '=', $param['relate_id'] ]
            );
            $order_info = $order_model->getLevelOrderInfo($order_condition)[ 'data' ];
            if (empty($order_info)) {
            ...
            $shipping_list = [
                [
                    ...
                    'item_desc' => $weapp_model->handleOrderShippingItemDesc([$order_info['level_name']]), // 商品信息，例如：微信红包抱枕*1个，限120个字以内
                    'contact' => [
                        'consignor_contact' => '',
                        'receiver_contact' => ''
                    ]
