...
export default {
	...
	redirectTo(to, param, mode) {
		let url = to;
		let tabbarList = ['/pages/index/index', '/pages/goods/list', '/pages/order/list', '/pages/my/index', '/pages/member/list']
	...
	upload: function(params, callback) {
		...
		uni.chooseImage({
			count: params.number,//上传数量只接受number，注意数据类型
			...
			success: async (res) => {
				...
				!error && typeof callback == 'function' && callback(imgs);
			}
		});
	},
	//上传
	...
	getDefaultImage() {
		...
		if (defaultImg) {
			...
			defaultImg.default_goods_img = this.img(defaultImg.goods);
			defaultImg.default_headimg = this.img(defaultImg.head);
			defaultImg.default_shop_img = this.img(defaultImg.store);
			...
			defaultImg.default_store_img = this.img(defaultImg.store);
			defaultImg.default_brand_img = this.img(defaultImg.goods);
			return defaultImg;
		} else {
			return {
				default_goods_img: '',
	...
	uniappIsIPhoneX() {
		...
		// #ifdef MP
		if (systemInfo.model.search('iPhone X') != -1 || systemInfo.model.search('iPhone 11') != -1 || systemInfo.model.search('iPhone12') != -1) {
			isIphoneX = true;
		}
		// #endif
