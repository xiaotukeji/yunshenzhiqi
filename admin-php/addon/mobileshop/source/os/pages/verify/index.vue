<template>
	<view class="verify iphone-safe-area">
		...
		<view class="scan">
			...
			<image :src="$util.img('public/uniapp/shop_uniapp/verify/verify.png')" mode="aspectFit" @click="scanCode()"></image>
			<view class="tip color-tip">点击此区域扫描核销码</view>
		</view>
		<!--  #endif -->
		<!-- #ifdef  H5 -->
		<view class="scan" v-if="$util.isWeiXin()">
			<!-- <view class="recode color-base-text" @click="$util.redirectTo('/pages/verify/records')">核销记录</view> -->
			<image :src="$util.img('public/uniapp/shop_uniapp/verify/verify.png')" mode="aspectFit" @click="scanCode()"></image>
			<view class="tip color-tip">点击此区域扫描核销码</view>
		</view>
	...
	methods: {
		...
		scanCode() {
			...
			uni.scanCode({
				onlyFromCamera: true,
				success: res => {
					this.verify_code = JSON.stringify(res);
					if (res.scanType == 'WX_CODE' && res.errMsg == 'scanCode:ok') {
						...
						this.verify_code = code;
						this.search();
					} else if (res.scanType == 'CODE_128' && res.errMsg == 'scanCode:ok') {
						try {
							var code =res.result;
							this.verify_code = code;
							this.search();
					...
					success: jssdkRes => {
						if (jssdkRes.code == 0) {
							...
							wxJS.scanQRCode(res => {
								if (res.resultStr) {
									let code = res.result;
									if ((code.indexOf('http://') != -1 || code.indexOf('https://') != -1) && code.indexOf('pages_tool/verification/detail') != -1){
										this.verify_code = this.getQueryVariable(code, 'code');
										this.search();
									}else if(code.indexOf('CODE_128') != -1){
										this.verify_code = code.split(',')[1];
										this.search();
									}else{
										this.$util.showToast({
											title: '无法识别'
