	...
	<view class="iphone-safe-area">
		<view class="item-wrap">
			<view class="form-wrap">
				...
				<input class="uni-input" placeholder="请输入网站名称" v-model="shopInfo.site_name" maxlength="100" />
			</view>
			...
			<view class="form-wrap goods-img">
				<text class="label">方形logo</text>
				<view class="img-list">
					<view class="add logo" @click="uplodImg('logo_square')">
						<text class="iconfont iconadd1" v-if="!shopInfo.logo_square"></text>
						<image v-else :src="$util.img(shopInfo.logo_square)" mode="aspectFit" @error="imgError('logo_square')" @click.stop="previewMedia('logo_square')"></image>
						<view class="del-wrap iconfont iconclose" @click.stop="delImg('logo_square')" v-if="shopInfo.logo_square"></view>
					</view>
					<view class="tips">建议图片尺寸：80*80</view>
				</view>
			</view>
			<!-- <view class="form-wrap goods-img">
				<text class="label">网站头像</text>
				<view class="img-list">
			...
			<view class="form-wrap more-wrap">
				...
				<textarea class="uni-input font-size-base" placeholder="请输入网站描述" v-model="shopInfo.seo_description" maxlength="300" auto-height="true"></textarea>
			</view>
			<view class="form-wrap more-wrap">
				<text class="label" style="align-self: flex-start;">网站关键字</text>
				<view class="flex_1">
					<input class="uni-input" placeholder="请输入网站关键字,用逗号隔开" v-model="shopInfo.seo_keywords" maxlength="100" />
...
<script>
import {setShopConfig} from '@/api/config'
export default {
	data() {
		return {
			shopInfo: {}
	...
	computed: {
		formData() {
			return {
				...
				logo_square: this.shopInfo.logo_square,
				...
				site_name: this.shopInfo.site_name
			};
		}
	},
	methods: {
		save() {
			setShopConfig(this.formData).then(res=>{
				this.$util.showToast({
					title: res.message
				});
				if (res.code >= 0) {
			...
			this.$util.upload(
				...
				res => {
					if (res) {
						...
						else if (type == 'logo_square') this.shopInfo.logo_square = res[0];
		...
		delImg(type) {
			...
			else if (type == 'logo_square') this.shopInfo.logo_square = '';
