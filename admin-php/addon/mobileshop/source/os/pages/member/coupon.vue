		...
		<mescroll-uni @getData="getCouponData" refs="mescroll" :size="10">
			<block slot="list">
				<block v-if="couponData.length">
					<view class="coupon-item" v-for="(item, index) in couponData" :key="index" :style="{ backgroundImage: 'url(' + $util.img('public/uniapp/shop_uniapp/member/coupon_bg.png') + ')' }" @click="isChecked(item)">
						...
						<view class="checkbox-wrap"><text class="checkbox iconfont" :class="item.checked ? 'iconfuxuankuang1 color-base-text' : 'iconfuxuankuang2'"></text>
						</view>
					</view>
				</block>
...
<script>
	import {getCouponLists,sendCoupon} from '@/api/coupon'
	export default {
		data() {
			return {
				emptyShow: false,
