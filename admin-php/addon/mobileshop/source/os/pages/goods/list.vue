	...
	<view class="list" @click="shwoOperation">
		<view class="search-wrap">
			<view class="search-input-inner">
				...
				<input class="uni-input font-size-tag" maxlength="50" v-model="formData.search_text" placeholder="请输入商品名称/商品编码" @confirm="searchGoods()" />
				<!-- #ifdef  MP-WEIXIN -->
				<text class="weixincode iconfont iconrichscan_icon" @click.stop="scanCode"></text>
				<!--  #endif -->
				<!-- #ifdef  H5 -->
				<text v-if="$util.isWeiXin()" class="weixincode iconfont iconrichscan_icon" @click.stop="scanCode"></text>
				<!--  #endif -->
			</view>
			
			<view class="search-btn color-base-bg" @click.stop="linkSkip()">+</view>
		</view>
			...
			<block slot="list">
				<view class="item-inner" v-for="(item, index) in dataList" :key="index">
					<view class="item-wrap" @click.stop="shwoOperation(item)">
						<image class="item-img" :src="item.goods_image == '' ? $util.img($util.getDefaultImage().default_goods_img) : $util.img(item.goods_image.split(',')[0], { size: 'mid' })" @error="imgError(index)"/>
						<view class="item-desc">
							<view class="item-name">
								<view class="name uni-using-hide">{{ item.goods_name }}</view>
								<text class="color-base-text goods-class">{{ item.goods_class_name }}</text>
							</view>
					...
					<view class="operation" @click.stop="showHide(item)" v-if="item.is_off">
						<view class="operation-item" @click.stop="linkSkip(item)" v-if="[1,2,3].indexOf(item.goods_class) != -1">
							<image :src="$util.img('public/uniapp/shop_uniapp/goods/goods_list_01.png')" mode="aspectFit"></image>
							<text>编辑</text>
						</view>
						<view class="operation-item" v-if="item.goods_state == 1" @click.stop="offGoodsFn(item)">
							<image :src="$util.img('public/uniapp/shop_uniapp/goods/goods_list_02.png')" mode="aspectFit"></image>
							<text>下架</text>
						</view>
						<view class="operation-item" v-if="item.goods_state == 0" @click.stop="onGoodsFn(item)">
							<image :src="$util.img('public/uniapp/shop_uniapp/goods/goods_list_06.png')" mode="aspectFit"></image>
							<text>上架</text>
						</view>
						<view class="operation-item" @click.stop="$util.redirectTo('/pages/goods/goodscard/list', {goods_id: item.goods_id})" v-if="item.goods_class == 5">
							<image :src="$util.img('public/uniapp/shop_uniapp/goods/goods_list_07.png')" mode="aspectFit"></image>
							<text>卡项</text>
						</view>
						<view class="operation-item" @click.stop="$util.redirectTo('/pages/goods/verify', {goods_id: item.goods_id})" v-if="[2,4].indexOf(item.goods_class) != -1">
							<image :src="$util.img('public/uniapp/shop_uniapp/goods/goods_list_07.png')" mode="aspectFit"></image>
							<text>核销码</text>
						</view>
						<view class="operation-item" @click.stop="goOutput(item)" v-if="item.goods_class != 3">
							<image :src="$util.img('public/uniapp/shop_uniapp/goods/goods_list_05.png')" mode="aspectFit"></image>
							<text>库存管理</text>
						</view>
					</view>
				</view>
