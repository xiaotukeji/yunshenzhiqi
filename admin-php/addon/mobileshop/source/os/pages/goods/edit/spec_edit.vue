		...
		<view class="sku-list">
			<view class="item-inner" v-for="(item, index) in goodsSkuData" :key="index">
				<view class="item-wrap" :class="{ 'item-shrink': item.shrink }">
					...
					<view class="form-wrap" v-if="goodsClass != 3">
						<view class="label">
							<text class="required color-base-text">*</text>
							<text>库存</text>
						</view>
					...
					<view class="form-wrap price" v-if="virtual_deliver_type == 'verify'">
						<view class="label">
							<text class="required color-base-text">*</text>
							<text>核销次数</text>
						</view>
						<input class="uni-input" v-model="item.verify_num" type="number" placeholder="0" />
					...
					<view class="form-wrap more-wrap" @click="openCarmichaelEdit(item)" v-if="goodsClass == 3">
						<text class="label">卡密管理</text>
						<input class="uni-input" type="text" :placeholder="item.carmichaelLength ? item.carmichaelLength : '添加卡密'" disabled="" />
...
export default {
	data() {
		return {
			// 商品类型:1 实物商品 ，2 虚拟商品
			goodsClass: 1,
			is_need_verify: 0, //是否核销
			virtual_deliver_type: '', //收货方式
			goodsSpecFormat: [],
			goodsSkuData: [],
			tempGoodsSkuData: [],
	...
	onLoad(option) {
		...
		this.is_need_verify = option.is_need_verify || 0;
		this.virtual_deliver_type = option.virtual_deliver_type || '';
	...
	onShow() {
		...
		this.goodsSkuData.forEach((item, key) => {
			...
			if (uni.getStorageSync('specName') == item.spec_name) {
				if (!uni.getStorageSync('editGoodsCarmichael')) {
					// item.carmichael = []
				} else {
					let carmichael = JSON.parse(uni.getStorageSync('editGoodsCarmichael'));
					item.carmichael = carmichael ? carmichael : [];
					if (carmichael.length > 0) {
						item.carmichaelLength = '添加卡密' + '【' + carmichael.length + '】';
					} else {
						item.carmichaelLength = '添加卡密';
					}
				}
			}
			if (item.sku_images) {
	...
	methods: {
			...
			for (var i = 0; i < this.tempGoodsSkuData.length; i++) {
				for (var j = 0; j < this.goodsSkuData.length; j++) {
					if (this.tempGoodsSkuData[i].spec_name == this.goodsSkuData[j].spec_name) {
						// this.goodsSkuData[j].carmichaelLength = '添加卡密'
		...
		verify() {
			...
			for (var i = 0; i < this.goodsSkuData.length; i++) {
				...
				if (item.stock.length == 0 && this.goodsClass != 3) {
					this.$util.showToast({ title: `请输入[第${i + 1}个规格]的库存` });
					flag = false;
					break;
				}
				if (this.goodsClass != 3) {
				...
				if (item.verify_num.length == 0 && this.virtual_deliver_type == 'verify') {
					this.$util.showToast({ title: `请输入[第${i + 1}个规格]的核销次数` });
					flag = false;
					break;
				}
				...
				if (this.goodsClass == 1) {
					if (isNaN(item.weight) || !this.$util.data().regExp.float3.test(item.weight)) {
						this.$util.showToast({ title: `[第${i + 1}个规格的重量kg]格式输入错误` });
						flag = false;
						break;
					}

					if (isNaN(item.volume) || !this.$util.data().regExp.float3.test(item.volume)) {
		...
		//编辑卡密
		openCarmichaelEdit(item) {
			if (item.carmichael) {
				uni.setStorageSync('editGoodsCarmichael', JSON.stringify(item.carmichael));
			} else {
				uni.setStorageSync('editGoodsCarmichael', '');
			}
