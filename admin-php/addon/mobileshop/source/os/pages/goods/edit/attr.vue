...
<script>
import {getAttrClassList,getAttributeListById} from '@/api/goods'
export default {
	data() {
		return {
			goodsAttrClass: 0,
	...
	onLoad(option) {
		...
		this.getAttrClassListFn();
	},
	onShow() {},
	methods: {
		// 获取商品参数
		getAttrClassListFn() {
			getAttrClassList().then(res=>{
				if (res.data) {
		...
		// 获取商品属性列表
		getAttributeList() {
			getAttributeListById(this.goodsAttrClass).then(res=>{
				if (res.data) {
