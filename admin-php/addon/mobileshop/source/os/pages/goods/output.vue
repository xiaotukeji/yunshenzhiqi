		...
		<view class="sku-list">
			<view class="item-wrap" v-for="(item, index) in outputList" :key="index">
				<view class="form-wrap">
					...
					<text class="value">{{ item.spec_name||item.sku_name }}</text>
				</view>
				<view class="form-wrap">
...
<script>
import {getOutputListById,editOutputList} from '@/api/goods'
export default {
	data() {
		return {
			goodsId: 0,
	...
	methods: {
		getOutputList() {
			getOutputListById(this.goodsId).then(res=>{
		...
		save() {
			...
			editOutputList({ sku_list: JSON.stringify(this.outputList) }).then(res=>{
				this.$util.showToast({
					title: res.message
				});
				if (res.code == 0) {
