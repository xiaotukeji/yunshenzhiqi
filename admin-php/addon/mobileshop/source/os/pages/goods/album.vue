	...
	<view>
		<view class="album-wrap">
			<scroll-view scroll-y="true" class="group-wrap">
				<block  v-for="(item, index) in albumList" :key="item.album_id">
					<block v-if="item.child_list && item.child_list.length > 0">
						<view class="item level1" >
							<text>{{ item.album_name }}</text>
						</view>
						<view class="item" :class="{ selected: child.album_id == albumId }" v-for="(child, itemIndex) in item.child_list" :key="child.album_id" @click="changeAlbum(child.album_id)">
							<text :class="{ 'color-base-border': child.album_id == albumId }">{{ child.album_name }}</text>
						</view>
					</block>
					<block v-else>
						<view class="item"  :class="{ selected: item.album_id == albumId }" @click="changeAlbum(item.album_id)">
...
<script>
import {getAlbumLists,getAlbumPicLists} from '@/api/album'
export default {
	data() {
		return {
			albumList: [],
	...
	onLoad(option) {
		this.number =Number( option.number) || 9;
	...
	methods: {
		...
		photograph() {
			this.$util.upload(
				{
					number: this.number,//上传数量只接受number，注意数据类型
					path: 'album',
					sourceType: ['camera'],
					album_id: this.albumId
				},
