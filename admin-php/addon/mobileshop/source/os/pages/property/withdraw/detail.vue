...
<!-- 		<view class="dl">
		...
		<view class="dl">
			...
			<view class="dd">{{ base_info.realname }}</view>
		</view>
		<view class="dl">
		...
		<block v-else>
			<!-- <view class="dl">
				...
				<view class="dd">{{ base_info.account_name }}</view>
			</view> -->
			<view class="dl">
				<view class="dt">支付宝账号</view>
				<view class="dd">{{ base_info.account_number }}</view>
			</view>
		</block>
		<view class="dl">
			<view class="dt">申请提现金额</view>
			<view class="dd">{{ base_info.apply_money }}元</view>
		</view>
		<view class="dl">
			<view class="dt">提现手续费</view>
			<view class="dd">{{ base_info.service_money }}元</view>
		</view>
		<view class="dl">
			<view class="dt">提现到账金额</view>
		...
		<view class="dl" style="border:none;">
...
<!-- 		<view class="dl">
		...
		<view class="menu_title" style="border:none;">
			<text class="line color-base-bg margin-right"></text>
			转账信息
		</view>
		<!-- <view class="dl">
			<view class="dt">转账凭证</view>
			<view class="menm_contimg"><image style="width: 100%;" class="img" :src="$util.img(base_info.certificate)" @click="previewMedia()" /></view>
		</view> -->
		<view class="dl" style="align-items: center;">
			<view class="dt">转账凭证</view>
			<view class="dd"><image class="img" :src="$util.img(base_info.certificate)" @click="previewMedia()" /></view>
		</view>
		
		<view class="dl" style="border:none;">
			<view class="dt">转账凭证说明</view>
			<view class="dd">{{ base_info.paying_money_certificate_explain ? base_info.paying_money_certificate_explain : '' }}</view>
		</view>
...
export default {
	...
	methods: {
		...
		previewMedia() {
			var paths = [this.$util.img(this.base_info.certificate)];
			uni.previewImage({
				current: 0,
				urls: paths
			});
