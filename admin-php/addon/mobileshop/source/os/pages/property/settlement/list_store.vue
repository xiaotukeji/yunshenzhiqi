								...
								<view class="withdrawal_list_info">
									...
									<view class="withdrawal_list_base">
										...
										<view class="color-base-text">
											￥{{ (parseFloat(item.order_money) - parseFloat(item.refund_shop_money) - parseFloat(item.commission)).toFixed(2) }}
										</view>
									</view>
									<view class="withdrawal_list_base">
										<view class="tip">线下结算金额</view>
										<view class="color-base-text">
											￥{{ (parseFloat(item.offline_order_money) - parseFloat(item.offline_refund_money)).toFixed(2) }}
										</view>
									</view>
									<view class="withdrawal_list_base">
		...
		<uni-popup ref="settleaccounts">
			<view class="pop-wrap" @touchmove.prevent.stop>
				<view class="title font-size-toolbar">结算备注
					<view class="close color-tip" @click="closePopup('settleaccounts')">
						<text class="iconfont iconclose"></text>
					</view>
				</view>
				<view class="textarea color-line-border">
					<textarea placeholder="请输入结算备注" placeholder-class="font-size-base" v-model="remark"></textarea>
				</view>
