		...
		<view class="tab-block">
			<scroll-view scroll-x="true" class="scroll tab-wrap" :scroll-into-view="'tab'+orderConditionCurr.refund_status_list" scroll-with-animation="true">
				<block v-for="(item, index) in upperStatusList" :key="index">
					<!-- #ifdef MP -->
					<view class="tab-item" @click="tabChange(index)" :class="{'active color-base-text color-base-bg-before':index == orderConditionCurr.refund_status_list,iphone:!$util.isAndroid()}" :id="'tab'+index">
						<text>{{ item.name ? item.name : '全部' }}</text>
					</view>
					<!-- #endif -->
					<!-- #ifndef MP -->
					<view class="tab-item" @click="tabChange(index)" :class="{'active color-base-text color-base-bg-before':index == orderConditionCurr.refund_status_list}" :id="'tab'+index">
						<text>{{ item.name ? item.name : '全部' }}</text>
					</view>
					<!-- #endif -->
				</block>
					...
					<view class="order-item" v-for="(item, index) in dataList" :key="index" @click="toDetail(item)">
						...
						<view class="total-wrap">
							...
							<view class="price color-base-text">
								<text class="unit">￥</text>
								<text>{{  (Number(item.refund_status == 3 ? item.refund_real_money : item.refund_apply_money) + Number(item.shop_active_refund_money)).toFixed(2) }}</text>
							</view>
						</view>
