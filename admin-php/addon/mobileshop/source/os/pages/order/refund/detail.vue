<template>
	<view class="order-detail-wrap">
		...
		<view class="block-wrap tit-auto">
			...
			<block v-if="detail.refund_apply_money > 0">
				<view class="order-cell">
					<text class="tit">维权类型：</text>
					<view class="box color-base-text">{{ detail.refund_mode > 1 ? '售后' : '退款' }}</view>
				</view>
				<view class="order-cell">
				...
				<view class="order-cell">
					...
					<view class="box">￥{{ detail.refund_apply_money }}</view>
				</view>
				<view class="order-cell">
					<text class="tit">申请原因：</text>
					<view class="box">{{ detail.refund_reason || '--' }}</view>
				</view>
				<view class="order-cell">
					<text class="tit">申请说明：</text>
					<view class="box">{{ detail.refund_remark || '--' }}</view>
				</view>
				<view class="order-cell" v-if="detail.refund_images.length > 0">
					<text class="tit">退款图片：</text>
					<view class="box image">
						<view v-for="(refund_image, refund_image_index) in detail.refund_images" @click="previewRefundImage(refund_image_index)">
							<image :src="$util.img(refund_image)"></image>
						</view>
					</view>
				</view>
				<block v-if="detail.refund_status == 3">
					<view class="order-cell-line"></view>
					<view class="order-cell">
						<text class="tit">退款金额：</text>
						<view class="box color-base-text">
							￥{{ detail.refund_real_money }} （{{ detail.shop_active_refund_money_type_name }}）
						</view>
					</view>
					<view class="order-cell">
		...
		<view class="block-wrap tit-auto">
			<view class="title">商家主动退款</view>
			<block v-if="detail.shop_active_refund == 1">
				<view class="order-cell">
					<text class="tit">退款编号：</text>
					<view class="box">{{ detail.shop_active_refund_no }}</view>
				</view>
				<view class="order-cell">
					<text class="tit">退款金额：</text>
					<view class="box color-base-text">
						￥{{ detail.shop_active_refund_money }} （{{ detail.shop_active_refund_money_type_name }}）
					</view>
				</view>
				<view class="order-cell">
					<text class="tit">退款说明：</text>
					<view class="box">{{ detail.shop_active_refund_remark }}</view>
				</view>
			</block>
			<block v-else>
				<view class="order-cell">
