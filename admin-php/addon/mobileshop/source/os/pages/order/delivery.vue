...
<script>
import {getOrderInfoById,getOrderGoodsList,deliveryOrder} from '@/api/order'
import {getExpressCompanyList} from '@/api/express'
export default {
	data() {
		return {
			order: {},
	...
	async onLoad(option) {
		...
		this.getOrderGoodsListFn();
	},
	onShow() {},
	methods: {
		// 获取订单信息
		async getOrderInfo() {
			var res = await getOrderInfoById(this.data.order_id);
		...
		// 获取订单项
		getOrderGoodsListFn() {
			getOrderGoodsList(this.data.order_id).then(res=>{
				if (res.code == 0) {
		...
		save() {
			...
			this.orderGoodsList.forEach(item => {
				...
				if((item.delivery_status && item.delivery_status_name == '未发货') || (this.order.is_lock && item.delivery_status_name == '未发货')) {
					this.data.order_goods_ids.push(item.order_goods_id);
				}
			});
			...
			deliveryOrder(this.data).then(res=>{
				if (res.code == 0) {
					this.$util.showToast({
						title: '发货成功'
					});
		...
		imgError(index) {
			this.orderGoodsList[index].sku_image = this.$util.getDefaultImage().default_goods_img;
			this.$forceUpdate();
		},
	}
};
