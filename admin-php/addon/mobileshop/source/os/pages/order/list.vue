<template>
	<view class="order-list-wrap">
		...
		<view class="search-wrap">
			<view class="search-input-inner" style="width: 100% !important;">
				<text class="search-input-icon iconfont iconsousuo" @click="searchOrder()"></text>
				<input class="search-input-text font-size-tag" maxlength="40" v-model="formData.search" placeholder="搜索订单号/商品名称/手机号/收货人姓名" @confirm="searchOrder()" />
			</view>
		</view>
		<view class="tab-block">
			<scroll-view scroll-x="true" class="scroll tab-wrap" :scroll-into-view="'tab' + orderConditionCurr.order_status_list" scroll-with-animation="true">
				...
				<block v-for="(item, index) in orderCondition.order_status_list" :key="index">
					<view class="tab-item" @click="tabChange(index + 1)"
				...
				<view class="item-wrap">
					...
					<view class="list">
						<block v-for="(item, index) in orderCondition.order_type_list" :key="index">
							<uni-tag :inverted="true" :text="item.name" :type="index == orderConditionCurr.order_type_list ? 'primary' : 'default'" @click="uTag(index, 'order_type_list', 'order_type')" />
						</block>
					</view>
				</view>
				<view class="item-wrap">
					...
					<view class="list">
						<uni-tag :inverted="true" text="全部" :type="orderConditionCurr.order_status_list == 0 ? 'primary' : 'default'" @click="uTag(0, 'order_status_list', 'order_status')" />
						<block v-for="(item, index) in orderCondition.order_status_list" :key="index">
							<uni-tag :inverted="true" :text="item.name" :type="index + 1 == orderConditionCurr.order_status_list ? 'primary' : 'default'" @click="uTag(index + 1, 'order_status_list', 'order_status')" />
						</block>
					</view>
				</view>
				<view class="item-wrap">
					...
					<view class="list">
						<uni-tag :inverted="true" text="全部" :type="orderConditionCurr.promotion_type == 0 ? 'primary' : 'default'" @click="uTag(0, 'promotion_type', 'promotion_type')" />
						<block v-for="(item, index) in orderCondition.promotion_type" :key="index">
							<uni-tag :inverted="true" :text="item.name" :type="index + 1 == orderConditionCurr.promotion_type ? 'primary' : 'default'" @click="uTag(index + 1, 'promotion_type', 'promotion_type')" />
						</block>
					</view>
				</view>
				<view class="item-wrap">
					...
					<view class="list">
						...
						<block v-for="(item, index) in orderCondition.pay_type_list" :key="index">
							<uni-tag :inverted="true" :text="item.name" :type="orderConditionCurr.pay_type_list == index + 1 ? 'primary' : 'default'" @click="uTag(index + 1, 'pay_type_list', 'pay_type')" />
						</block>
					</view>
				</view>
				<view class="item-wrap">
					...
					<view class="list">
						<uni-tag :inverted="true" text="全部" :type="orderConditionCurr.order_from_list == 0 ? 'primary' : 'default'" @click="uTag(0, 'order_from_list', 'order_from')" />
						<block v-for="(item, index) in orderCondition.order_from_list" :key="index">
							<uni-tag :inverted="true" :text="item.name" :type="orderConditionCurr.order_from_list == index + 1 ? 'primary' : 'default'" @click="uTag(index + 1, 'order_from_list', 'order_from')" />
						</block>
					</view>
				</view>
				<!-- <view class="item-wrap">
				...
				<block v-if="orderList.length > 0">
					<view class="order-item" v-for="(item, index) in orderList" :key="index" @click="toDetail(item)">
						<view class="head">
							...
							<text class="pay-type-name color-base-text">{{ item.order_type_name }}</text>
						...
						<view class="goods-item" v-for="(goods, goodsIndex) in item.order_goods" :key="goodsIndex">
							<view class="goods-img">
								<image :src="$util.img(goods.sku_image, { size: 'mid' })" mode="widthFix" @error="imgError(index, goodsIndex)"/>
							</view>
							<view class="info-wrap">
								<view class="name-wrap">{{ goods.goods_name }}</view>
								<view class="spec-wrap" v-if="goods.sku_spec_format">
									<block v-for="(x, i) in goods.sku_spec_format" :key="i">{{ x.spec_value_name }}
										{{ i < goods.sku_spec_format.length - 1 ? '; ' : '' }}
									</block>
								</view>
								<view class="more-wrap">
									<view class="left-wrap">
										<view class="goods-class">{{ goods.goods_class_name }}
											<text v-if="item.order_type == 1 && item.order_status == 1 && goods.delivery_status == 1" class="color-base-text delivery-status">已发货</text>
										</view>
										<text class="present-label color-base-bg" v-if="goods.is_present == 1">赠品</text>
										<view class="goods-class" v-if="goods.sku_no">UPC {{ goods.sku_no }}</view>
									</view>
									<view class="price-wrap">
										...
										<view class="subtotal">小计 ￥{{ goods.num * goods.price | moneyFormat }}</view>
									</view>
								</view>
							</view>
						...
						<view class="member-wrap" v-if="item.order_scene == 'online'">
							...
							<view class="address" v-if="item.order_type == 2 && item.buyer_ask_delivery_time_str">预提货时间：{{ item.buyer_ask_delivery_time_str }}</view>
							<view class="address" v-if="item.order_type == 3 && item.buyer_ask_delivery_time_str">配送时间：{{ item.buyer_ask_delivery_time_str }}</view>
							<view v-if="item.order_type != 4" class="copy-sty" @click.stop="copyAddressMob(item)">复制</view>
						</view>
						<view class="remark-wrap color-base-bg-light color-base-text" v-if="item.buyer_message">买家留言：{{ item.buyer_message }}</view>
						<view class="remark-wrap color-base-bg-light color-base-text" v-if="item.remark">卖家备注：{{ item.remark }}</view>
						<view class="displayflex" v-if="item.order_status == 0">
							<view class="displayflex-text positionr" @click.stop="itemBtn(index)">
								更多
								<view class="positiona" v-if="btnIndex == index">
									<view class="boxshaow" style="position: relative;">
										<view class="triangle-up"></view>
										<view class="boxshaow-btn">
											<view @click.stop="orderRemark(item)">备注</view>
											<view v-if="item.order_status == 0 && item.order_scene == 'online'" @click.sotp="offlinePay(item.order_id)">线下支付</view>
										</view>
									</view>
								</view>
							</view>
							<view class="order-action-wrap">
								<button type="primary" size="mini"
									...
									:key="actionIndex" @click.stop="orderAction(actionItem.action, item.order_id)">
									{{ actionItem.title }}
								</button>
						...
						<view class="order-action-wrap" v-else>
							<button type="primary" size="mini" plain @click.stop="orderRemark(item)">备注</button>
							<button type="primary" size="mini"
								v-for="(actionItem, actionIndex) in item.order_status_action.action" :key="actionIndex"
								@click.stop="orderAction(actionItem.action, item.order_id)">
								{{ actionItem.title }}
							</button>
							<button v-if="item.order_status == 0" type="primary" size="mini" @click.sotp="offlinePay(item.order_id)">线下支付</button>
							<button v-if="item.order_type == 2 && item.order_status == 2" type="primary" size="mini" @click.stop="storeOrderTakedeliveryFn(item.order_id)">
								提货
							</button>
						</view>
					</view>
