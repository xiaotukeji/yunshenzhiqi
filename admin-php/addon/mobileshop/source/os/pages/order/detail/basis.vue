<template>
	<view class="order-detail-wrap">
		...
		<view class="status-wrap color-base-bg" :style="{ backgroundImage: 'url(' + $util.img('public/uniapp/order/status-wrap-bg.png') + ')' }">
			<view class="container">
				...
				<view class="status-name">
					...
					<template v-if="orderDetail.promotion_status_name">
						({{ orderDetail.promotion_status_name }})
					</template>
				</view>
			</view>
		</view>
		...
		<!-- 地址信息 -->
		<view class="address-wrap" v-if="orderDetail.order_scene == 'online'">
		...
		<view class="block-wrap">
			<view v-for="(goods, goodsIndex) in orderDetail.order_goods" :key="goodsIndex" class="goods-item-wrap">
				<view class="goods-item">
					...
					<view class="info-wrap">
						...
						<view class="more-wrap">
							<view class="goods-class">{{ goods.goods_class_name }}
								<text v-if="orderDetail.order_type == 1 && orderDetail.order_status == 1 && goods.delivery_status == 1" class="color-base-text delivery-status">已发货</text>
							</view>
						...
						<!-- <view class="delivery-status-name color-base-text" v-if="goods.is_virtual == 0">{{ goods.delivery_status_name }}</view> -->
					</view>
					...
					<view class="action-wrap" v-else-if="orderDetail.is_enable_refund == 1 && orderDetail.promotion_type != 'blindbox' && goods.shop_active_refund == 0">
						<button type="primary" size="mini" @click.stop="shopActiveRefund(goods.order_goods_id)">主动退款</button>
					</view>
				</view>
				<view class="goods-form" v-if="goods.form">
					<view class="order-cell" v-for="(item, index) in goods.form" :key="index">
						...
						<view class="box" v-else>
							...
							<text class="copy color-base-text" @click="$util.copy(item.val)">复制</text>
						</view>
					</view>
				</view>
			</view>
		...
		<view class="block-wrap order-form" v-if="orderDetail.form">
			<view class="order-cell" v-for="(item, index) in orderDetail.form" :key="index">
				<text class="tit">{{ item.value.title }}：</text>
				<view class="box img-box" v-if="item.controller == 'Img'">
					<view class="img" v-for="(img, imgIndex) in item.img_lists" :key="imgIndex">
						<image :src="$util.img(img)" mode="widthFix" @click="previewMedia($util.img(img))"></image>
		...
		<view class="block-wrap">
			...
			<view class="order-cell" v-if="orderDetail.store_id">
				<text class="tit">来源门店：</text>
				<view class="box">{{ orderDetail.store_name || '--' }}</view>
			</view>
			...
			<view class="order-cell">
				...
				<view class="box">{{ orderDetail.member_id ? orderDetail.nickname : '散客' }}</view>
				<view class="copy color-base-text" v-if="orderDetail.member_id" @click="$util.redirectTo('/pages/member/detail', { member_id : orderDetail.member_id})">查看</view>
			</view>
		...
		<view class="block-wrap">
			...
			<view class="order-cell" v-if="orderDetail.member_card_money > 0">
				<text class="tit">会员卡</text>
				<view class="box align-right">
					<text class="color-base-text">
						<text class="font-size-goods-tag">￥</text>
						<text>{{ orderDetail.member_card_money }}</text>
					</text>
				</view>
			</view>
			<view class="order-cell" v-if="orderDetail.invoice_money > 0">
				<text class="tit">
					税费
					<text class="color-base-text">({{ orderDetail.invoice_rate }}%)</text>
				</text>
				<view class="box align-right">
					<text class="color-base-text">
						<text class="operator">+</text>
						<text class="font-size-goods-tag">￥</text>
						<text>{{ orderDetail.invoice_money }}</text>
					</text>
				</view>
			</view>
			<view class="order-cell" v-if="orderDetail.invoice_delivery_money > 0">
				<text class="tit">发票邮寄费</text>
				<view class="box align-right">
					<text class="color-base-text">
						<text class="operator">+</text>
						<text class="font-size-goods-tag">￥</text>
						<text>{{ orderDetail.invoice_delivery_money }}</text>
					</text>
				</view>
			</view>
			...
			<view class="order-cell" v-if="orderDetail.point_money > 0">
			...
			<view class="order-cell">
				<view class="box align-right money bold">
					<text>共{{ orderDetail.goods_num }}件商品，实付金额：</text>
					<text class="font-size-goods-tag color-base-text">￥</text>
					<text class="font-size-base color-base-text">{{ orderDetail.pay_money }}</text>
				</view>
			</view>
		</view>
			...
			<view class="item" v-for="(item, index) in log" :key="index">
				<view class="action">
					<view class="title">
						...
						<text class="color-sub time">{{ $util.timeStampTurnTime(item.action_time, 'Y-m-d') }}</text>
					</view>
					<view>
						<text class="color-tip">操作人：{{ item.nick_name }}</text>
						<br />
						<text class=" color-tip">操作时间：{{ $util.timeStampTurnTime(item.action_time) }}</text>
						<br />
