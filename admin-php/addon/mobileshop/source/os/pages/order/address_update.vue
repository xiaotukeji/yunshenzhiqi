...
import {getAddressInfo,editAddress} from '@/api/address';
export default {
	data() {
		return {
			orderId: 0,
	...
	methods: {
		getOrderInfo() {
			getOrderInfoById(this.orderId).then(res=>{
				if (res.code == 0) {
					this.order = res.data;
		...
		save() {
			...
			if (this.verify()) {
				...
				if (this.order.order_type == 3) {
					data.latitude = this.order.latitude;
					data.longitude = this.order.longitude;
				}
