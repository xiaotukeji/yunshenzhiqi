...
<template>
	<view class="chartsview" :id="'ChartBoxId'+cid">
		...
		<block v-else>
			<view @tap="rdcharts.tap" @mousemove="rdcharts.mouseMove" @mousedown="rdcharts.mouseDown"
				@mouseup="rdcharts.mouseUp" @touchstart="rdcharts.touchStart" @touchmove="rdcharts.touchMove"
				@touchend="rdcharts.touchEnd" :id="'UC'+cid" :prop="uchartsOpts" :change:prop="rdcharts.ucinit">
				<canvas :id="cid" :canvasId="cid"
					:style="{ width: cWidth + 'px', height: cHeight + 'px', background: background }"
					:disable-scroll="disableScroll" @error="_error" v-if="showchart" />
			</view>
		</block>
		<!-- #endif -->
		<!-- 支付宝小程序 -->
		...
		<block v-if="ontouch">
			<canvas :id="cid" :canvasId="cid" :width="cWidth * pixel" :height="cHeight * pixel"
				...
				@touchend="_touchEnd" @error="_error" v-if="showchart" />
		</block>
		<block v-if="!ontouch">
			<canvas :id="cid" :canvasId="cid" :width="cWidth * pixel" :height="cHeight * pixel"
				:style="{ width: cWidth + 'px', height: cHeight + 'px', background: background }"
	...
	export default {
		...
		data() {
			return {
				...
				echartsResize: false,
				uchartsOpts: {
					canvasId: "ngWYfF8K5Nr4cB760dzmDRw2QLF6tvHm"
				},
				echartsOpts: {},
				drawData: {},
				lastDrawTime: null,
