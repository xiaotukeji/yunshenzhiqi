...
export default {
  ...
  methods: {
      ...
      payment(this.orderCreateData).then(res => {
        ...
        if (code >= 0 && data) {
          if (data.delivery.express_type && data.delivery.express_type.length) {
            data.delivery.express_type = data.delivery.express_type.filter(
    ...
    // 查询优惠券
    getCouponListFn() {
      if (this.modules.indexOf('coupon') == -1) return;
      getCouponList(this.orderCreateData).then(res => {
        const {
          code,
          message,
          data
    ...
    promotion(data) {
      if (data.promotion && data.promotion.manjian && data.promotion.manjian.length) {
        ...
        let manjianInfo = data.promotion.manjian[0];
            ...
            if (item.coupon && item.coupon_data) {
              ...
              item.coupon_data.forEach((couponItem, couponIndex) => {
                if (couponItem.type == 'discount') {
                  if (text == '') text = '送' + item.coupon_num[couponIndex] + '张' + parseFloat(couponItem
                    .discount) + '折优惠券';
                  else text += '、送' + item.coupon_num[couponIndex] + '张' + parseFloat(couponItem.discount) +
                    '折优惠券';
                } else {
                  if (text == '') text = '送' + item.coupon_num[couponIndex] + '张' + parseFloat(couponItem
                    .money) + '元优惠券';
                  else text += '、送' + item.coupon_num[couponIndex] + '张' + parseFloat(couponItem.money) +
                    '元优惠券';
                }
              })
    ...
    orderCalculate() {
      ...
      calculate(data).then(res => {
        ...
        if (code >= 0 && data) {
          ...
          if (!data.coupon_id) this.orderCreateData.coupon.coupon_id = 0;
    ...
    openPlatformCoupon() {
      ...
      this.couponList.forEach(function (e, i) {
        if (e.coupon_id == that.orderCreateData.coupon.coupon_id) {
          row = e
        }
      })
