...
class Turntable extends BaseApi
{
    ...
    public function info()
    {
        ...
        $game = new Games();
        $info = $game->getGamesInfo([['game_id', '=', $game_id], ['site_id', '=', $this->site_id], ['game_type', '=', 'turntable']], 'game_id,game_name,points,start_time,end_time,status,remark,no_winning_desc,no_winning_img,is_show_winner,level_id,level_name,join_type,join_frequency');
        if (!empty($info[ 'data' ])) {
            // 奖项
            $game_ward = $game->getGameAward([['game_id', '=', $game_id]], 'award_id,award_name,award_img')[ 'data' ];
            ...
            if ($info[ 'data' ][ 'join_frequency' ] && $token[ 'code' ] == 0) {
                $info[ 'data' ][ 'surplus_num' ] = $game->getMemberSurplusNum($game_id, $this->member_id, $this->site_id)[ 'data' ];
            }
        }
