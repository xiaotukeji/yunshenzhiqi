...
class ShowPromotion
{
        ...
        $data = [
            ...
            'shop' => [
                [
                    ...
                    'title' => '红包裂变',
                    //展示介绍
                    'description' => '帮助商家提升用户量',
                    //展示图标
                    'icon' => 'addon/hongbao/icon.png',
                    //跳转链接
    ...
    private function summary($params)
    {
        ...
        if (isset($params[ 'count' ])) {
            $count = model("promotion_hongbao")->getCount([ [ 'site_id', '=', $params[ 'site_id' ] ] ]);
            return [
                'count' => $count
            ];
        }
        ...
        if (isset($params[ 'summary' ])) {
            $list = model("promotion_hongbao")->getList([
                [ '', 'exp', Db::raw('not ( (`start_time` >= ' . $params[ 'end_time' ] . ')  or (`end_time` <= ' . $params[ 'start_time' ] . '))') ],
                [ 'site_id', '=', $params[ 'site_id' ] ],
                [ 'status', '<>', -1 ]
            ], 'name as promotion_name,hongbao_id as promotion_id,start_time,end_time');
            return !empty($list) ? [
                'time_limit' => [
                    'count' => count($list),
                    'detail' => $list,
