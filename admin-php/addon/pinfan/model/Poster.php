...
class Poster extends BaseModel
{
    ...
    public function goods($app_type, $page, $qrcode_param, $promotion_type, $site_id)
    {
        try {
            ...
            $pintuan_info = $pinfan_model->getPinfanInfo([ [ 'pintuan_id', '=', $goods_info[ 'pintuan_id' ] ] ])[ 'data' ];
            #活动剩余时长
            $residue_time = $pintuan_info[ 'end_time' ] - time();

            if ($residue_time < 86400) {//如果不到一天
                $time = explode(' ', gmstrftime('%H %M %S', $residue_time));
                $format_time = $time[ 0 ] . ' : ' . $time[ 1 ] . ' : ' . $time[ 2 ];
            } else {
                $time = explode(' ', gmstrftime('%j %H %M %S', $residue_time));
                $format_time = ( $time[ 0 ] - 1 ) . ' 天 ' . $time[ 1 ] . ' : ' . $time[ 2 ] . ' : ' . $time[ 3 ];
            }
    ...
    private function getGoodsInfo($id)
    {
        ...
        $field = 'ngs.sku_name,ngs.introduction,ngs.sku_image,ngs.sku_id,nppg.pintuan_price,nppg.pintuan_id,ngs.template_id,ngs.price';
        $info = model('promotion_pinfan_goods')->getInfo([ 'nppg.id' => $id ], $field, $alias, $join);
        return $info;
    }
