...
layuiXtree.prototype.dataBind = function (d, level=0) {
    var _this = this;
    if (d.length > 0) {
        level++;
        for (i in d) {
            ...
            _this._domStr += '<div class="layui-xtree-item" data-tag="iem" data-level="' + level + '" data-child="'+xtree_child+'">';

            _this._domStr += '<input type="checkbox" class="layui-xtree-checkbox" ' + xtree_isend + xtree_ischecked + xtree_isdisabled + ' value="' + d[i].value + '" title="' + d[i].title + '" lay-skin="primary" lay-filter="xtreeck' + _this._containerid + '" data-input-level="' + level + '" data-child="'+xtree_child+'">';
            _this.dataBind(d[i].data, level);
            _this._domStr += '</div>';
        }
    }
}
...
layuiXtree.prototype.Rendering = function () {
    ...
    for (var i = 0; i < xtree_items.length; i++) {
        if (xtree_items[i].parentNode == _this._container){
        	xtree_items[i].style.padding = '0px 0 8px 10px';
        }  
        else {
            ...
            var item_length = 0;
             ...
             if (xtree_items[i].childNodes.length > 0) {
            
                 for(ii in xtree_items[i].childNodes){
                     var dom = xtree_items[i].childNodes[ii];
                     if(typeof(dom) == 'object') {
                         if(dom.getAttribute("data-child") > 0){
                             for(t in dom.parentNode.childNodes){
                                 var childDom = dom.parentNode.childNodes[t];
                                 if(typeof (childDom) == 'object'){
                                     if(childDom instanceof HTMLElement){
                                         if(childDom.getAttribute("class") == "layui-xtree-item" && childDom.getAttribute("data-child") == 0){
                                             childDom.style.display = 'inline-block';
                                             childDom.setAttribute('is_three','1');
                                         }
                                         if (childDom.getAttribute("class") == "layui-unselect layui-form-checkbox" ){
                                             childDom.style.display = 'table';
                                         }
                                     }
                                 }
                             }
            ...
            if(xtree_items[i].getAttribute("data-child") > 0){
                for(t in xtree_items[i].childNodes){
                    var childDom = xtree_items[i].childNodes[t];
                    if (typeof (childDom) == 'object') {
                        if (childDom instanceof HTMLElement) {
                            if (childDom.getAttribute("class") == "layui-xtree-item" && childDom.getAttribute("data-child") == 0) {
                                childDom.style.display = 'inline-block';
                            ...
                            if (childDom.getAttribute("class") == "layui-unselect layui-form-checkbox" || childDom.getAttribute("class") == "layui-unselect layui-form-checkbox layui-form-checked") {
                ...
                //二级分类块排序
                 if (xtree_items[i].getAttribute("data-level") == "2" && xtree_items[i].getAttribute("data-child") == 0) {
                     for(t in xtree_items[i].parentNode.childNodes){
                         var childDom = xtree_items[i].parentNode.childNodes[t];
                         if (typeof (childDom) == 'object') {
                             if (childDom instanceof HTMLElement) {
                                 if (childDom.getAttribute("class") == "layui-xtree-item" && childDom.getAttribute("data-child") == 0) {
                                     childDom.style.display = 'inline-block';
