  ...
  Cascader.prototype = {
    ...
    _initInput: function () {
      ...
      this.$input = $('<div class="el-input el-input--suffix">' +
        '<input type="text" readonly="readonly" autocomplete="off" placeholder="' + placeholder + '" class="layui-input layui-unselect">' +
        '<span class="el-input__suffix">' +
        '<span class="el-input__suffix-inner">' +
        '<i class="el-icon-arrow-down ' + fromIcon + ' ' + downIcon + '" style="font-size: 16px;margin-right: 4px;"></i>' +
        '</span></span>' +
        '</div>')
      this.$div.append(this.$input);
      this.$inputRow = this.$input.find('.layui-input');
      // 多选标签
      if (multiple) {
    ...
    close: function (eventId) {
      if (this.showPanel && (!eventId || this.closeEventId === eventId)) {
        this.showPanel = false;
        this.$div.find('.layui-icon-triangle-d').removeClass('is-reverse');
    ...
    open: function () {
      if (!this.showPanel) {
        ...
        this._resetXY();
        // 箭头icon翻转
        this.$div.find('.layui-icon-triangle-d').addClass('is-reverse');
