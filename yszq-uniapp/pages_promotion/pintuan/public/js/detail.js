export default {
	...
	methods: {
		...
		// 发起拼团
		//createGroup true 发起拼团 false 参与拼团
		pintuan(createGroup = true,groupId) {
			let group_id = createGroup ? 0 : groupId ? groupId : this.groupId;
			if (!this.storeToken) {
				// 分享参数、链接
				this.shareQuery = 'pintuan_id=' + this.pintuan_id;
				if(!createGroup) this.shareQuery += '&group_id=' + group_id;
				if(this.source_member) this.shareQuery += '&source_member=' + this.source_member;
				this.shareUrl = this.goodsRoute + '?' + this.shareQuery;
			...
			if (this.groupDetail && this.groupDetail.status == 2 && this.groupDetail.order_id) {
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							that.$util.redirectTo('/pages/order/detail', {
								'order_id': that.groupDetail.order_id
							});
						} else if (res.cancel) {
		...
		openPinTuan(groupId, pintuan_num, time, headimg, head_id) {
			if (this.memberInfo && this.memberInfo.member_id == head_id) {
				this.$util.showToast({
					title: "您不能参与自己的团"
				});
				return;
